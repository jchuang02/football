...
export const updateFixtures =
  (season, league) => async (dispatch, getState) => {
    if (Date.now() - getState().fixtures[league].lastUpdated >= 60000) {
      const { data } = await football.get("/fixtures", {
        params: { season, league },
      });
...
export const updateLiveFixturesById =
  (id, league) => async (dispatch, getState) => {
    if (Date.now() - getState().fixtures[league].lastUpdated >= 60000) {
      const { data } = await football.get("/fixtures", {
        params: { id },
      });
...
export const updateTeamFixtures =
  (team, season) => async (dispatch, getState) => {
...
export const updateLiveTeamFixturesById =
  (id, team) => async (dispatch, getState) => {
    if (Date.now() - getState().teamFixtures[team].lastUpdated >= 60000) {
      const { data } = await football.get("/fixtures", {
        params: { id },
      });
